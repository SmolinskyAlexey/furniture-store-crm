 ...
 * CRM Refunds Wizard - Модуль создания заказов v2.1
...
class RefundsWizard {
    ...
    async init() {
        ...
        const RefundsPage = document.getElementById("refunds-page");
        ...
        try {
            ...
            //
            this.refundsPagination();//10.06.25
            this.initializeFilters();
            this.attachSearchListeners();

        } catch (error) {
            console.error("Ошибка инициализации RefundsWizard:", error);
    ...
    async preloadData() {
        ...
        try {
            [refunds] = await Promise.all([
                window.queries.searchRefundsQuery(
                    (this.currentRefundsPage-1) * this.cardsPerPage,
                    this.cardsPerPage,
                    this.cache.filter
                    )
            ]);
            
            this.cache.refunds = refunds;
            this.mountRefunds(refunds);
        } finally {
            this.showLoading(false);
        }
    }

    mountRefunds(refunds){
        const html = window.RefundsTemplates.renderRefunds(refunds.list);
        ...
        totalLabel.innerHTML = "Всего "+refunds.total_num+" заказов";
        this.refundsPagination();//обновляем пагинацию
    }
    ...
    attachSearchListeners() {
        ...
        const handleSearch = async () => {
            ...
            try {
                ...
                this.currentRefundsPage = 1;
                this.mountRefunds(this.cache.refunds);
    ...
    refundsPagination(){
        ...
        const totalRefunds = this.cache.refunds?.total_num;

        const totalPages = Math.ceil(totalRefunds / this.cardsPerPage);
        ...
        prev.addEventListener("click", () => {
                if (this.currentRefundsPage > 1) {
                    this.currentRefundsPage = this.currentRefundsPage - 1;
        ...
        next.addEventListener("click", () => {
                if (this.currentRefundsPage < totalPages) {
                    this.currentRefundsPage = this.currentRefundsPage + 1;
        ...
        pageIndicator.innerHTML = `стр ${this.currentRefundsPage}&nbsp;из ${totalPages}`;
...
window.RefundsWizard = RefundsWizard;
